...
if isscalar(check)
    ...
    toggle.histogram = 1;           % Toggles PD histogram exhibition
    toggle.h0_h1_pdf = 0;
...
folder = 'trash';                   % Folder in which the results are saved

runs = 40000;                       % Number of Monte Carlo events
th_points = 300;                    % Number of threshold points to generate of the ROC curves
<<<<<<< HEAD
param.snr_dB = -15;       % Mean SNR (dB)
=======
param.snr_dB = -20;       % Mean SNR (dB)
>>>>>>> 1f1c705966d2338d09a380f9187044608e10c73f
snr = 10.^(param.snr_dB/10);        % SNR conversion to linear

% SS parameters
%param.ss.m = 1;                    % Number of Cognitive Radios (for cooperative, not implemented)
<<<<<<< HEAD
param.ss.n = 1e7;                 % Number of samples collected for each spectrum sensing period
param.ss.tech = 2;                 % Spectrum sensing Technique: (0) ED, (1) SRD, (2) CRD, (3) PDP-Var, (4) GED, (5) Hybrid ED/CRD,
=======
param.ss.n = 1000;                 % Number of samples collected for each spectrum sensing period
...
param.ch.ris_toggle = 1;            % (0) Only Tx-Rx channel, no RIS; (1) RIS present
param.ch.elements = 8;              % Number of RIS reflective elements
...
param.ch.tx_rx = 0;                % Channel model for the direct Tx-Rx channel, put (-1) for no direct path
...
% Practical problems
param.nu_dB = 0;                    % Noise uncertainty [dB] (the same for the whole set of samples)
nu = 10.^(param.nu_dB/10);          % Noise uncertainty
param.freq_error_var = 50;
param.freq_error_mean = 0;%fs*0.1;
param.in.type = 0;                  % IN type: (0) None, (1) Type I, (2) Type II, (3) Laplacian,
                                    % (4) Alpha-Stable, (5) Sub-gaussian
% Laplacian noise
param.in.lapl.b = 3;
...
for i = 1 : runs
    ...
    if i > runs/2                               % Under H_1 Tx signal is generated
        ...
        if param.ch.ris_toggle == 0                 % No RIS, only Direct Tx-Rx Channel
            H = fading(param.ch.tx_rx-1, 1, param.ch.parameter, param.ch.name, param.ch, 0, 0);
        else                                        % RIS enabled
            H_ris = 0; H0 = 0;
            for l = 1:param.ch.elements
                H1 = fading(param.ch.tx_ris-1, 1, param.ch.parameter, param.ch.name, param.ch, param.ch.delta, l);   % Tx-RIS
                H2 = fading(param.ch.ris_rx-1, 1, param.ch.parameter, param.ch.name, param.ch, param.ch.delta, l);   % RIS-Rx
            ...
            if param.ch.tx_rx ~= -1                 % Direct link also present
                H0 = fading(param.ch.tx_rx, 1, param.ch.parameter, param.ch.name, param.ch, 0, 0);
            end
            H = H0 + H_ris;
        end
    end
    ...
    % Detection
    if param.ss.tech == 5 || param.ss.tech == 6 || param.ss.tech == 9 % Hybrid detection
        [T1,T2] = detection_hyb(signal_rx, param.ss, i, toggle.histogram, param.tx.fc, fs, runs, snr, param.freq_error);
        if i<= runs/2
            T1_H0(i)  = T1;
            T2_H0(i) = T2;
        else
